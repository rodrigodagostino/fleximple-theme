...
( function () {
	...
	const itemsWithChildren = mainNav.querySelectorAll(
		'.menu-item-has-children',
	)

	for ( let i = 0; i < itemsWithChildren.length; i++ ) {
		const submenuToggle = itemsWithChildren[ i ].querySelector(
			'.submenu-toggle',
		)
	...
	window.addEventListener( 'scroll', function () {
		...
		for ( let i = 0; i < sections.length; i++ ) {
			...
			if (
				currentPos >= sectionTop - 10 &&
				currentPos <= sectionBottom - 10
			) {
				...
				if ( currentMenuItem ) {
					currentMenuItem.parentElement.classList.add(
						'current-menu-item',
					)
				}
			}
